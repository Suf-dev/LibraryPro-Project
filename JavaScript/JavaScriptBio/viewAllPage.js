...
function getBook(){
    fetch('http://localhost:9001/displayAll')
    .then(
      function(response) {
        ...
        response.json().then(function(data) {
          ...
          for (let i = 0; i < data.length; i++) {
            document.querySelector("#view").innerHTML = data[i].bid;
            console.log(
              (document.querySelector("#view").innerHTML = data[i].bname)
            );
            console.log(
            ...
            para.innerText = `Book id : ${(document.querySelector(
              "#view"
             ).innerHTML = data[i].bid)} \n Title :  ${(document.querySelector(
              "#view"
             ).innerHTML = data[i].bname)} \n  Author : ${(document.querySelector(
              "#view"
