...
function getBook(){
    fetch('http://localhost:9001/showAll')
    .then(
      function(response) {
        ...
        response.json().then(function(data) {
          ...
          for (let i = 0; i < data.length; i++) {
            document.querySelector("#view").innerHTML = data[i].tid;
            console.log(
              (document.querySelector("#view").innerHTML = data[i].tname)
            );
            console.log(
            ...
            para.innerText = `Book id : ${(document.querySelector(
              "#view"
             ).innerHTML = data[i].tid)} \n Title :  ${(document.querySelector(
              "#view"
             ).innerHTML = data[i].tname)} \n  Author : ${(document.querySelector(
              "#view"
